...
def objective(trial, final_data, var_names):
    ...
    TRIAL_number_of_layers = trial.suggest_int("num_layers",1,8)
    TRIAL_model_dim = trial.suggest_categorical("model_dim", [16, 32, 64, 128, 256])
    TRIAL_number_of_heads = trial.suggest_categorical("num_heads", [1, 2, 4, 8, 16])
    TRIAL_num_epochs = trial.suggest_int("num_epochs",30, 150,15)
...
def get_final_data(dataset_name, filename):
    ...
    pastPointsToForecast = PAST_POINTS_TO_FORECAST
    numberFolds=3
...
def main():
    ...
    for filename in filenames:
        ...
        with open("results_transformer_fmri.txt","a") as f:
